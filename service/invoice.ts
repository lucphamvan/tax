  ...
  const detailRows = data.hdhhdvu
    ?.map((item) =>
      [
        ...
        `"${item.ten}"`,
        item.dvtinh || "",
        item.sluong?.toString() || "",
        item.dgia?.toString() || "",
        item.stckhau || "",
        item.ltsuat || "",
        item.thtien?.toString() || "",
      ].join(",")
    )
    .join("\n");
...
const buildDetailInvoiceData = async (
...
const fetchInvoices = async (
  ...
  const { data } = await axios.get<GetListInvoiceResponse>(
    GET_LIST_INVOICE_URL,
    {
      params,
      headers: {
        Authorization: "Bearer " + token,
...
const retrieveAllInvoices = async (
  ...
  const { datas, state: newState } = await fetchInvoices(
    token,
    from,
    to,
    state
